    ...
    extends TestBase
{
    ...
    public void doListTables() throws Exception
    {
        ...
        for (TableName next : res.getTables()) {
            ...
            assertTrue("Table name should be in the expected list", tableNames.contains(next.getTableName()));
        }
        assertEquals(tableNames.size(), res.getTables().size());
    }

    @Test
    public void testDoListTables_withPagination() throws Exception
    {
        Document tableNamesDocument = new Document("cursor",
                new Document("firstBatch",
                        Arrays.asList(new Document("name", "table1"),
        ...
        // Verify first page
        assertEquals(2, firstResponse.getTables().size());
        ...
        // Verify second page
        assertEquals(2, secondResponse.getTables().size());
        ...
        // Verify final page
        assertEquals(1, finalResponse.getTables().size());
        ...
        assertEquals(0, emptyResponse.getTables().size());
            ...
            throws Exception
    {
        ...
        TableName tableNameInput = new TableName("deFAULT", TEST_TABLE.toUpperCase());
        GetTableRequest req = new GetTableRequest(IDENTITY, QUERY_ID, DEFAULT_CATALOG, tableNameInput, Collections.emptyMap());
        caseInsensitiveHandler.doGetTable(allocator, req);
    }
            ...
            throws Exception
    {
        ...
        assertTrue("Partition row count should be greater than 0", partitions.getRowCount() > 0);
    }
    ...
    public void doGetSplits()
    {
        ...
        assertEquals("Continuation criteria violated - splits size should be 1", 1, response.getSplits().size());
        assertNull("Continuation criteria violated - continuation token should be null", response.getContinuationToken());
    }
