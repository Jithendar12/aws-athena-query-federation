...
public class SynapseJdbcConnectionFactoryTest
{
    ...
    {
        try (Connection connection = createConnection(MOCK_JDBC_URL + ";${test}", 
                new StaticCredentialsProvider(new DefaultCredentials(TEST_USERNAME, TEST_PASSWORD)))) {
            String expectedUrl = MOCK_JDBC_URL + ";user=" + TEST_USERNAME + ";password=" + TEST_PASSWORD;
    ...
    public void testGetConnection_withActiveDirectoryServicePrincipal()
    {
        try (Connection connection = createConnection(
                MOCK_JDBC_URL + ";authentication=ActiveDirectoryServicePrincipal;${test}",
                new StaticCredentialsProvider(new DefaultCredentials(TEST_USERNAME, TEST_PASSWORD)))) {
            String expectedUrl = MOCK_JDBC_URL + ";authentication=ActiveDirectoryServicePrincipal;AADSecurePrincipalId=" + 
                    TEST_USERNAME + ";AADSecurePrincipalSecret=" + TEST_PASSWORD;
            assertEquals(expectedUrl, actualUrl.get());
            assertEquals(mockConnection, connection);
        }
        catch (Exception e) {
    ...
    {
        try (Connection connection = createConnection(MOCK_JDBC_URL + ";user=" + TEST_USERNAME + ";password=" + TEST_PASSWORD, null)) {
            assertEquals(MOCK_JDBC_URL + ";user=" + TEST_USERNAME + ";password=" + TEST_PASSWORD, actualUrl.get());
            assertEquals(mockConnection, connection);
        }
        catch (Exception e) {
            fail("Unexpected exception: " + e.getMessage());
