...
public class AwsRestHighLevelClientFactoryTest
{
    ...
    @Test
    public void getOrCreateClient_calledTwice_returnsSameCachedClient()
    {
        ...
        assertNotNull("First client from cache with AWS credentials should not be null", client1);
        assertNotNull("Second client from cache with AWS credentials should not be null", client2);
    ...
    @Test
    public void getOrCreateClient_withUsernameAndPassword_calledTwice_returnsCachedClientInstance()
    {
        // Use a unique endpoint for this test to avoid conflicts with other tests
        String uniqueEndpoint = "https://unique-test-" + System.currentTimeMillis() + ".us-east-1.es.amazonaws.com";
        ...
        assertNotNull("First client from cache with username and password should not be null", client1);
        assertNotNull("Second client from cache with username and password should not be null", client2);
        // Note: Cache may evict unhealthy clients, so we just verify both calls return a client
        // The actual caching behavior is tested in CacheableAwsRestHighLevelClientTest
    }
}
